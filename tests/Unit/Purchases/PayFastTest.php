...
class PayFastTest extends TestCase
{
    ...
    public function create_a_checkout_form_from_an_order()
    {
        ...
        $order_data = [
            'merchant_id'        => '12345',
            'merchant_key'       => 'abcde',
            'return_url'         => config('app.url') . '/payfast/return/' . $order->order_key,
            'cancel_url'         => config('app.url') . '/payfast/cancel/' . $order->order_key,
            'notify_url'         => (config('app.url') ?? config('app.url')) . '/payfast/notify/' . $order->order_key,
