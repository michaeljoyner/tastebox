...
$factory->state(Menu::class, 'current', function () {
    $day = ((now()->dayOfWeek > 4) || (now()->dayOfWeek === 0)) ? now()->addWeek() : now();

    return [
        'current_from'  => $day->startOfWeek(),
        'current_to'    => $day->startOfWeek()->addDays(5),
        'delivery_from' => $day->endOfWeek()->addDay(),
        'can_order'     => true,
    ];
}
);
